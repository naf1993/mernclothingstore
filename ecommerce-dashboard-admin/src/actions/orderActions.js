...
export const getOrdersByUserId = (userId) =>async(dispatch,getState)=>{
    try{
        ...
        const config = getConfig(getState)
        const {data} = await axios.get('/api/orders/user',{_id:userId},config)
...
export const updateOrderStatus = (orderId, status) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.patch(`/api/orders/${orderId}`, { status }, config);
...
export const getOrderSummary = () => async (dispatch, getState) => {
    try {
        ...
        dispatch({ type: ORDER_SUMMARY_FAIL, payload: error.response.data.message });
    }
};

// Get sales data
export const getSalesData = (startDate, endDate) => async (dispatch, getState) => {
    try {
        dispatch({ type: SALES_DATA_REQUEST });

        const config = getConfig(getState); // Use the reusable config function

        const { data } = await axios.get(`/api/orders/sales?startDate=${startDate}&endDate=${endDate}`, config);
