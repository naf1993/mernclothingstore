...
import { addToWishList, createProduct, deleteProduct ,getAllProducts, getProductById, getProductStatistics, getProductsByCategory, getProductsBySubCategory, getSimilarProducts, productSearch, resizeCoverImage, resizeImages, updateProduct, uploadCoverImage, uploadFiles } from '../controllers/productController.js';
...
router.route('/').get(getAllProducts).post(protect,restrictToAdmin,uploadCoverImage,resizeCoverImage,createProduct)
router.route('/:id').get(getProductById).patch(protect,restrictToAdmin,uploadFiles,resizeImages,updateProduct).delete(protect,restrictToAdmin,deleteProduct)
